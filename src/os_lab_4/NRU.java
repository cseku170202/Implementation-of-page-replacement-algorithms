...
public class NRU {
    ...
    String a[],inframe[];
    int r[],m[];
    NRU(int num_pages, String a[],int num_frame)
    {
        ...
        this.r=new int[this.num_frame];
        this.m=new int[this.num_frame];
    }
        ...
        for(i=0;i<num_pages;i++) // i is column
        {
            ...
            else
            {
                ...
                if(j==num_frame)
                {
                    ...
                    for(j=0;j<num_frame;j++)
                    {
                        System.out.println("Enter page Reference and Modify for frame "+(j+1)+" containing page "+inframe[j]);
                        r[j]=sc.nextInt();
                        m[j]=sc.nextInt();
                        int t=2*r[j]+m[j];
                        if(t<low)
